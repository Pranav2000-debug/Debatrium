...
export const DesktopSidebar = ({ className, children, ...props }) => {
  ...
  return (
    <motion.div
      className={cn("h-full px-4 py-4 hidden md:flex md:flex-col bg-neutral-100 dark:bg-neutral-800 w-[300px] shrink-0", className)}
      animate={{
...
export const MobileSidebar = ({ className, children, ...props }) => {
  ...
  return (
    <div className={cn("h-14 px-4 flex md:hidden items-center justify-between bg-neutral-100 dark:bg-neutral-800 w-full", className)} {...props}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={cn("fixed inset-0 z-[100] bg-white dark:bg-neutral-900 p-10 flex flex-col justify-between", className)}>
            <div className="absolute right-10 top-10 z-50" onClick={() => setOpen(false)}>
              <IconX />
            </div>
            {children}
          </motion.div>
